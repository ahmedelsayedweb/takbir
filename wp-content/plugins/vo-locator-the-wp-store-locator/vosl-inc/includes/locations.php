...
<input type="text" class="vosl-form-control" name="place_address" id="place_address" placeholder="<?php echo __("Enter address or zipcode", VOSL_TEXT_DOMAIN); ?>">
...
?>
<div class="col-lg-4 voslsingletag">
...
<?php /*?><?php if($result_map_center[0]['id'] > 0){ ?>
	initializeLocationMap('<?php echo $result_map_center[0]['latitude']?>','<?php echo $result_map_center[0]['longitude']?>');
<?php }else{ ?>
	initializeLocationMap('<?php echo $result_voslrow[0]['latitude']?>','<?php echo $result_voslrow[0]['longitude']?>');
<?php } ?><?php */?>
...
timedOut=false;
timerId=setTimeout(function(){timedOut=true;$("#btnFind").click();$("#loadingIcon").hide();},10000);

$("#loadingIcon").show();
if(navigator.geolocation){
bvoslrowserSupportFlag=true;
navigator.geolocation.getCurrentPosition(function(position){ voslBrowserGeolocationSuccess(position, $); },function(error){ voslBrowserGeolocationFail(error, $); });}
...
<?php
//do_action('vosl_reinitializemap');
