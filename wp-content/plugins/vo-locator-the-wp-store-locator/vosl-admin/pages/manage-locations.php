...
<table id="vosl_manage_listings_dt" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th><?php echo __("Phone", VOSL_TEXT_DOMAIN); ?></th>
	...
	vosl_listing_admin_table = jQuery('#vosl_manage_listings_dt').DataTable( {
		...
		"createdRow": function ( row, data, index ) {
            if ( data[7] == '' ) {
				...
				jQuery('td', row).eq(7).html('<i class="fa text-yellow fa-exclamation-triangle" title="<?php echo addslashes(__("Unable to find lattitude,longitude for this listing. Please set correct address.", VOSL_TEXT_DOMAIN));?>" aria-hidden="true"></i>');
            }
			//console.log(data);
        },
		"scrollX": true
